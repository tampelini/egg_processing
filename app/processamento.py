...
def processar_imagem(
    ...
    *,
    ...
    se utiliza a imagem calibrada ou a original.
    ...
    # 2) CÓPIA CRUA (antes de filtros) -> BASE (fonte de cores + render final)
    imagem_base = imagem_trabalho.copy()

    # 3) Cópia de trabalho: só para detecção
    ...
    # 4) Padronizar tamanho (aplica MESMO resize nas duas cópias)
    ...
    # 5) Ajustes finos SOMENTE na BASE
    ...
    # 6) Cópia para exibição (retorno final)
    imagem_exibicao = imagem_base.copy()

    # 7) (somente para detecção) CLAHE
    ...
    for linha in linhas:
        for (x, y, w, h), ctr in linha:
            ...
            if use_fit:
                (cx, cy), (Wf, Hf), ang = cv2.fitEllipse(ctr)
                eixo_x = int(max(1, round((Wf * 0.5) * fx)))
                eixo_y = int(max(1, round((Hf * 0.5) * fy)))
            ...
            # =========================================================
            # MUDANÇA: orientação usando o BOX VERDE (boundingRect w,h)
            # =========================================================
            ratio_box = w / (h + 1e-6)
            thr = 1.10  # ajuste: 1.05–1.20 (menor => menos "Indefinido")

            if ratio_box >= thr:
                orien = "Horizontal"
            elif ratio_box <= (1.0 / thr):
                orien = "Vertical"
            else:
                orien = "Indefinido"

            preview = todas_cores[:8]
            top_cores_preview_text = ", ".join(
                [f"{c['hex']} ({c['perc']:.1f}%) — {c['count']} px" for c in preview]
            )

            info = {
                "num": cnt,
                "centro": (int(centro_x), int(centro_y)),
                "eixo": (int(eixo_x), int(eixo_y)),
                "orientacao": orien,
            ...
            # ===== Anotações visuais — sempre na imagem_exibicao =====
            cv2.rectangle(imagem_exibicao, (x, y), (x + w, y + h), (0, 255, 0), 2)

            # =========================================================
            # MUDANÇA: "X" vermelho baseado na MESMA regra do orien
            # (antes era if w/h > 1.1, agora usa orien)
            # =========================================================
            if orien == "Horizontal":
    ...
    # 13) Codifica PNG a partir da imagem_exibicao
